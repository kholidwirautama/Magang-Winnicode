...
def manager_list(request):
    ...
    # Add group information for each manager
    manager_with_groups = []
    for mgr in manager:
        try:
            user = User.objects.get(username=mgr.utxt)
            groups = [group.name for group in user.groups.all()]
        except User.DoesNotExist:
            groups = []

        # Add groups attribute to manager object
        mgr.user_groups = groups
        manager_with_groups.append(mgr)

    return render(request, 'back/manager_list.html', {'manager':manager_with_groups})
