...
def news_detail(request,word):
    ...
    cat_list = []
    for c in cat:
        count = News.objects.filter(ocatid=c.pk, act=1).count()  # hanya berita publish
        cat_list.append({'name': c.name, 'count': count})
        ...
        try:
            ...
            if str(myfile.content_type).startswith("image"):
                if myfile.size < 5000000:## Check File Size ##
                    ...
                    # Jika user manager dan status sebelumnya confirmed, ubah ke pending
                    if perm == 0 and b.act == 1:
                        b.act = 0
                    b.save()
                    return redirect('news_list')
                else:
